...
export async function updateDelivery(data, id) {
  ...
  if (!data.is_cancelled && data.deliveryQuantity && data.delivery_item) {
    ...
    else if (data.deliveryQuantity && isNaN(Number(data.deliveryQuantity)))
      return { message: 'Quantity Must be a Number', ok: false };
    else if (data.amountType1 == 'AC' && !data.party_name)
      return { message: 'AC type Must have Party Name', ok: false };
    else if (data.amountType1 == 'AC' && data.amountType2 == 'AC')
      return { message: `Both amount type can't be AC`, ok: false };
  }

  const priceResult = calculateAmount(data.address, data.delivery_item, data.deliveryQuantity);
  if (!priceResult.success) {
    return { message: priceResult?.message, ok: false };
  }

  data.deliveryTime = getFormattedTime();
...
export async function updateDeliveryAmount(data, id) {
  if (Number(data.sign)) return { message: 'Can not edit Signed Delivery Record', ok: false };
  if (data.amount2 && isNaN(Number(data.amount2)))
    return { message: 'Amount 2 Must be a Number', ok: false };
  if (data.amountType1 == 'AC' && data.amountType2 == 'AC')
    return { message: `Both amount type can't be AC`, ok: false };

  const priceResult = await calculateAmount(
    data.address,
    data.delivery_item,
