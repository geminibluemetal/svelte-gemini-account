...
export function deliverySchema() {
  return z
    .object({
      ...
      isOwnerDelivery: z.coerce.boolean().default(false),
    ...
    .superRefine((data, ctx) => {
      ...
      if (data.amountType !== 'AC' && (!data.amount || data.amount === 0)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Amount is required for non-AC deliverys',
          path: ['amount'],
        });
      }
    });
...
export const deliveryCreateSchema = deliverySchema();
export const deliveryUpdateSchema = deliverySchema();
